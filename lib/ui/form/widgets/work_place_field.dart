...
class WorkPlaceField extends HookConsumerWidget {
  const WorkPlaceField({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final controller = useTextEditingController();
    controller.text =
        ref.read(lecturerFormControllerProvider).lecturer.workPlace;
      ...
      if (previous?.isEdting != next.isEdting) {
        next.formState.maybeWhen(
            ...
            loadFieldsDataSuccess: (v) {
              controller.text = (v as Lecturer).workPlace;
            });
      }
    });
